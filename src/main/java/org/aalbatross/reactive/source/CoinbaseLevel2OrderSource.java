...
@ClientEndpoint
public class CoinbaseLevel2OrderSource extends Endpoint
    implements FlowableOnSubscribe<Order>, Source {
  ...
  public void onMessage(String message, boolean last) {
    ...
    sb.append(message);
    if (last) {
      decoder.apply(sb.toString()).map(response -> new ResponseToOrders().apply(response)).stream()
          .flatMap(Collection::stream).forEachOrdered(order -> {
            if (emitter != null)
  ...
  public void stop() {
    try {
      unsubscribe();
      onClose(session,
          new CloseReason(CloseReason.CloseCodes.NO_STATUS_CODE, "Stopping the consumer"));
