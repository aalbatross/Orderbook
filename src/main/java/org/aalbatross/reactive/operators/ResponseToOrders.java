      ...
      response -> {
        var buyOrders = response.getBids().stream()
            .map(fields -> Order.builder().orderType(OrderType.BUY)
                .price(Double.parseDouble(fields.get(0))).size(Double.parseDouble(fields.get(1)))
                .build())
            .collect(Collectors.toList());
        var sellOrders = response.getAsks().stream()
            .map(fields -> Order.builder().orderType(OrderType.SELL)
                .price(Double.parseDouble(fields.get(0))).size(Double.parseDouble(fields.get(1)))
                .build())
            .collect(Collectors.toList());
  ...
  private final Function<Level2UpdateNotification, List<Order>> updateNotificationToOrders =
      response -> response.getChanges().stream()
          .map(change -> Order.builder()
              ...
              .price(Double.parseDouble(change.get(1))).size(Double.parseDouble(change.get(2)))
              .build())
          .collect(Collectors.toList());

  @Override
  public List<Order> apply(Level2Response response) {
    return Optional.ofNullable(response).map(Level2Response::getType).map(type -> {
